...
libs <- c('GA', 'dplyr', 'ggplot2','doParallel' ) #, 'tensorflow', 'keras',
...
Po = tf$constant(Po)
P = tf$constant(df$P)
r = tf$Variable(tf$zeros(shape(1L)))

Pi = tf$multiply(Po,tf$exp(tf$multiply(r,t)))
error = tf$reduce_sum(tf$square(tf$subtract(Pi,P)))

# modelObj <- tf_function(f = Pi)
...
evaln = compile(object = modelObj, optimizer = opt, loss = error)

evaln = evaluate(object = piblah, optimizer = opt, loss = error)

piblah <- df$P[df$t == 0]*exp(r*df$t)
...
# # We write a plot function to see the progress of the solutions in each generations
# monitor <- function(obj) 
# { 
#   curve(fitnessFunction, 0, 0.6, main = paste("iteration =", obj@iter))
